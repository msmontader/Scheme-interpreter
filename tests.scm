...
(define sum-variables (+ (+ x y) z))
; expect sum-variables

sum-variables
; expect 1965

'(denero is the best)
; expect (denero is the best)

(quote koreanfriedchicken)
; expect koreanfriedchicken

(cons (car '(hot cold)) (cdr '(cat dog)))
; expect (hot dog)

(begin (+ 6 9) (* 45 8) (/ (- 45897 502) 5))
; expect 9079

(lambda (a) (/ sum-variables a))
; expect (lambda (a) (/ sum-variables a))

(define (average-variables a) (/ sum-variables a))
; expect average-variables
...
(and average-variables sum-variables False)
; expect False

(and x (* 5 (+ 80 (- 1 1))) z (or False (- 1 1) True))
; expect 0

(and 1 69 nil True false (/ 1 0))
; expect False

(cond
  ((and y False) (cons average-variables 'koreanfriedchicken))
  (else (or (begin (+ 6 1)) False))
)
; expect 7

(let ((x 'yummy) (y (cons x (cons 'pizza nil)))) (list x y))
; expect (yummy (100 pizza))

(list x y)
; expect (100 555)

(define f (mu (c) (cons c x)))
; expect f

(define g (lambda (c e) (cons (f 'drink) (cons c e))))
; g

(g 'sierra 'nevada)
; expect ((drink . 100) sierra . nevada)

(define (evenlist s)
(cond
((null? s) nil)
((= (car s) 0) 0)
((even? (car s)) (cons (car s) (evenlist (cdr s))))
(else (evenlist (cdr s)))
))
(evenlist '(1 2 4 5 6 7 8))
; expect (2 4 6 8)

(define (remove-num x s)
(cond
((null? s) nil)
((pair? (car s)) (remove-num x (car s)))
((equal? (car s) x) (remove-num x (cdr s)))
(else (cons (car s) (remove-num x (cdr s))))
))
(remove-num 3 '(1 3 4 6 (3 4)))
; expect (1 4 6 4)
