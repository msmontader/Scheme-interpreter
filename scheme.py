...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("Different amount of values for formal parameters")
        else:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Cannot Call Function On These Arguments")
        # END PROBLEM 4
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        symbols, values = Pair(binding.first, symbols), Pair(scheme_eval(binding.second.first, env), values)
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            operands = rest.map(lambda rest: scheme_optimized_eval(rest, env))
